...
4. **Reporter is `testthat::SummaryReporter$new()`** (R6 object — no string names). Prepend `options(cli.width = 200); testthat::set_max_fails(Inf); ` to every `Rscript -e` body so failure descriptions aren't truncated and the run reports every failure. `SummaryReporter` is required because the parent uses the failing `test_that()` description to dispatch a precise `test_file(path, desc = "...")` rerun, and a truncated description forces a fallback to running the whole file.
...
Every `-e '...'` body starts with `options(cli.width = 200); testthat::set_max_fails(Inf); ` so `SummaryReporter` doesn't truncate descriptions and the run never stops early. Allow rules like `Bash(Rscript -e 'options(cli.width = 200); testthat::set_max_fails(Inf); testthat::test_local*)` and `Bash(Rscript -e 'options(cli.width = 200); testthat::set_max_fails(Inf); testthat::test_file*)` match cleanly.

**Single file** (substitute `<FILE>`, `<LOG>`):
`Rscript -e 'options(cli.width = 200); testthat::set_max_fails(Inf); testthat::test_file("<FILE>", reporter = testthat::SummaryReporter$new())' 2>&1 | tee -a "<LOG>"`

**List of files**: dispatch the single-file template once per file, in caller's order.

**Full suite** (substitute `<LOG>`):
`Rscript -e 'options(cli.width = 200); testthat::set_max_fails(Inf); testthat::test_local(reporter = testthat::SummaryReporter$new())' 2>&1 | tee -a "<LOG>"`

**Failure cap.** testthat caps reported failures at 10 by default. Always prepend `testthat::set_max_fails(Inf); ` inside the `-e '...'` body (after `options(...);`) so the run never stops early and every failure is reported.
